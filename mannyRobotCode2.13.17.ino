...
void loop() // the infinite loop
{
  while (x < 1) {
    for (int i = 0; i < RelayBoard::PINS;
      ...
      read_value(i, &readings,
                 &relays); //(enter the value from the pins struct?)
      Serial.print(
          "Reading from "); // prints the comment in serial data printout
  ...
  while (z < 1) {
    ...
    while (storingunswitchedSort < 1) {
      for (count = 0; count < 10; count++) {
        // repeat until i= length of list
        for (int i = 0; i < 5; i++) {
          if (unswitchedSort[i] < unswitchedSort[i + 1]) { // swap the two elements
            temp = unswitchedSort[i];
            unswitchedSort[i] = unswitchedSort[i + 1];
            unswitchedSort[i + 1] = temp;
          }
        }
        storingunswitchedSort++;
      }
    }
    ...
    while (intermidiateprint < 1) {
      ...
      Serial.println(unswitchedSort[0]);
      Serial.println(unswitchedSort[1]);
      Serial.println(unswitchedSort[2]);
      Serial.println(unswitchedSort[3]);
      Serial.println(unswitchedSort[4]);
